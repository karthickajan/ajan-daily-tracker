"""
Ajan Daily Tracker - Beautiful Web UI
A modern, beautiful daily habit tracker with a web interface
...
<body>
    ...
    <script>
        ...
        const SUPABASE_KEY = "sb_publishable_PAqRM1mxu_St436Be0NwVw_Aj7MW-kp";
        ...
        async function initializeSupabase() {
            ...
            try {
                ...
                if (data && data.length > 0) {
                    ...
                    trackerData.entries = data.map(entry => ({
                        date: entry.date,
                        sleep_6h: entry.sleep_6h,
                        sleep_hours: entry.sleep_hours,
                        bathed: entry.bathed,
                        hair_controlled: entry.hair_controlled,
                        ate_enough: entry.ate_enough,
                        protein_ok: entry.protein_ok,
                        dsa_studied: entry.dsa_studied,
                        dsa_hours: entry.dsa_hours,
                        sysdesign_studied: entry.sysdesign_studied,
                        sysdesign_hours: entry.sysdesign_hours,
                        deepwork_90min: entry.deepwork_90min,
                        solved_designed: entry.solved_designed,
                        low_distraction: entry.low_distraction,
        ...
        function saveData() {
            ...
            if (userId && trackerData.entries) {
                trackerData.entries.forEach(async (entry) => {
                    try {
...
def main():
    ...
    ╔═══════════════════════════════════════════════════════════╗
    ║                                                           ║
    ║   ✨ Ajan Daily Tracker - Beautiful UI Edition ✨         ║
    ║                                                           ║
    ║   Server running at: http://localhost:{port}               ║
    ║                                                           ║
    ║   Open the URL in your browser to start tracking!         ║
